...
def scatter(shap_values, color="#1E88E5", hist=True, axis_color="#333333", cmap=colors.red_blue,
    ...
    if interaction_index is not None:
        ...
        clow = np.nanpercentile(cv.astype(np.float_), 5)
        chigh = np.nanpercentile(cv.astype(np.float_), 95)
        if clow == chigh:
            clow = np.nanmin(cv.astype(np.float_))
            chigh = np.nanmax(cv.astype(np.float_))
        if type(cd[0]) == str:
            cname_map = {}
