    ...
    if len(shap_values.shape) == 3:
        ...
        for i in range(shap_values.shape[0]):
            ...
            if callable(explainer.masker.shape):
                all_on_masked = explainer.masker(np.ones(explainer.masker.shape(data[i])[1], dtype=np.bool_), data[i])
            else:
                all_on_masked = explainer.masker(np.ones(explainer.masker.shape[1], dtype=np.bool_), data[i])
