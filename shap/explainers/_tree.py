...
class XGBTreeModelLoader(object):
    ...
    def __init__(self, xgb_model):
        ...
        for i in range(self.num_trees):
            ...
            self.leaf_child_cnt.append(np.zeros(self.num_nodes[i], dtype=np.int_))
            for j in range(self.num_nodes[i]):
    ...
    def get_trees(self, data=None, data_missing=None):
        ...
        self.children_default = np.zeros(shape, dtype=np.int_)
        self.features = np.zeros(shape, dtype=np.int_)
