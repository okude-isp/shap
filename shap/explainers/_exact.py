...
class Exact(Explainer):
    ...
    def explain_row(self, *row_args, max_evals, main_effects, error_bounds, batch_size, outputs, interactions, silent):
        ...
        if getattr(self.masker, "clustering", None) is None:
            ...
            extended_delta_indexes = np.zeros(2**len(inds), dtype=np.int_)
            ...
            if interactions is False or interactions is 1: # pylint: disable=literal-comparison
                ...
                mask = np.zeros(len(fm), dtype=np.bool_)
...
def partition_delta_indexes(partition_tree, all_masks):
    ...
    mask = np.zeros(all_masks.shape[1], dtype=np.bool_)
    delta_inds = []
...
def gray_code_masks(nbits):
    ...
    """
    out = np.zeros((2**nbits, nbits), dtype=np.bool_)
    li = np.zeros(nbits, dtype=np.bool_)
...
def gray_code_indexes(nbits):
    ...
    """
    out = np.ones(2**nbits, dtype=np.int_) * MaskedModel.delta_mask_noop_value
    li = np.zeros(nbits, dtype=np.bool_)
